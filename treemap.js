const width6 = window.innerWidth;
const height6 = window.innerHeight;

const filePath = '/dataset/Treemap_redWine_data.json';
const animationSpeed = 500;

let data;
// let color = d3.scaleSequential([8, 0], d3.interpolateRgb.gamma(2.2)("purple", "orange"));
let color6 = d3.scaleOrdinal().range(d3.schemePurples[7]);
...
const render = data => {
    ...
    const node = newSvg.selectAll('g')
                       .data(d3.group(root, d => d.height6))
                       .join('g')
    ...
    // Create labels
    node.append('text')
         .attr("font-family", "Cutive Mono")
        .attr('clip-path', d => `url(#${d.clipId})`)
        .selectAll('tspan')
        .data(d => [d.data.Name,d.data.name, (d.value)])
        .join('tspan')
    ...
    // .ease(d3.easeCubicOut)
          ...
          .on('end', () => {
              ...
              svg6.attr('class', 'temp').selectAll('*').remove();
          });

    d3.select('select').on('change', function () {
        color6 = d3.scaleSequential([8, 0], d3[d3.select(this).property('value')]);

        node.select('rect').attr('fill', d => color6(d.height));
    });
};
